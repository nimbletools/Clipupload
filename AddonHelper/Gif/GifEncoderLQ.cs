...
namespace AddonHelper {
    public class GifEncoderLQ {
        private static readonly List<Color> OptimalPalette;
        private static readonly byte[] PaletteBytes;

        // TIL: Static constructors
        static GifEncoderLQ() {
            GifEncoderLQ.OptimalPalette = new List<Color>(256);

            // I made an console program that outputs this to a .txt. Don't judge me.
            ...
            PaletteBytes = new byte[256 * 3];
            for (int i = 0; i < 256; i++) {
                Color c = OptimalPalette[i];

                PaletteBytes[i * 3 + 0] = c.R;
        ...
        private static void Register(int red, int green, int blue) {
            GifEncoderLQ.OptimalPalette.Add(Color.FromArgb(255, red, green, blue));
        }

        public byte[] GetColorBytes() {
            return GifEncoderLQ.PaletteBytes;
        }
    }
}
