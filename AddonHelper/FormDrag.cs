    ...
    void SetArea()
    {
      ...
      { // Horizontal width picture
        ...
        for (int i = 0; i <= bmp.Width / 100; i++) {
          g.DrawLine(Pens.White, new Point(i * 100, bmp.Height / 4), new Point(i * 100, (bmp.Height / 4) * 3));
        }
      ...
      { // Vertical height picture
        ...
        for (int i = 0; i <= bmp.Height / 100; i++) {
          g.DrawLine(Pens.White, new Point(bmp.Width / 4, i * 100), new Point((bmp.Width / 4) * 3, i * 100));
        }

        g.TranslateTransform(0, !flipAround ? 40 : bmp.Height - 10);
        g.RotateTransform(-90);
        g.DrawString(this.Selection.Height.ToString(), SystemFonts.DefaultFont, Brushes.White, PointF.Empty);
